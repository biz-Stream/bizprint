...
```
/create-pr → PR 作成 → CI 監視 Monitor + pr-reviewer エージェント（並列起動）
  → pr-reviewer 完了 → 指摘ありなら /check-review を自動実行
  → 指摘対応 → コミット・プッシュ → CI 監視 Monitor + pr-reviewer エージェント（並列再起動）
...
- **`CI_RESULT: success`** / **`CI_RESULT: skipped`** → ユーザーに報告する。再レビューエージェントの完了を待つ。
- **`CI_RESULT: not_found`** → 該当 SHA のワークフローが作成されていない旨をユーザーに報告する。
- **`CI_RESULT: failure`** / **`CI_RESULT: cancelled`** → ユーザーに確認する。修正・再プッシュした場合は CI 監視 Monitor を再作成する。
